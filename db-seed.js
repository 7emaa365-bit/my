...
(function () {
  ...
  async function seedAll() {
    ...
    try {
      ...
      // seed the global key (legacy) and per-restaurant keys
      try {
        localStorage.setItem("Menu", JSON.stringify(sampleMenu));
      } catch (e) {
        console.warn("Failed write global Menu", e);
      }
      // list of slugs to seed (match script.js restaurant slugs)
      const slugs = [
      ...
      slugs.forEach((s, idx) => {
        try {
          // create a full branch object per slug
          const branch = {
            slug: s,
            name: s,
            logo: "",
            ...
            social: {},
            phone:
              sampleBranches[idx] && sampleBranches[idx].phone
                ? sampleBranches[idx].phone
                : "201021279663",
            menu: JSON.parse(JSON.stringify(sampleMenu)),
            orders: [],
            createdAt: Date.now(),
          };
          localStorage.setItem(`db_branch_${s}`, JSON.stringify(branch));
          // also write legacy Menu_ key for backward compatibility
          localStorage.setItem(`Menu_${s}`, JSON.stringify(sampleMenu));
        } catch (e) {
          console.warn("seed per-restaurant failed", s, e);
        }
      });
